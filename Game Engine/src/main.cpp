...
int main(int argc, char** argv)
{
    ...
    auto& aircraftRenderer = aircraft.getBodyRenderer();
    auto& birdLeftWingRenderer = aircraft.getLeftWingRenderer();
    auto& birdRightWingRenderer = aircraft.getRightWingRenderer();
    auto& treeRenderer = aircraft.getTreeRenderer();
    ...
    birdLeftWingRenderer.   setProjection(projection_matrix);
    birdRightWingRenderer.   setProjection(projection_matrix);
    treeRenderer.       setProjection(projection_matrix);
    ...
    birdLeftWingRenderer.setLightDirection({-0.3f, 0.3f, 0.9f});
    birdRightWingRenderer.setLightDirection({-0.3f, 0.3f, 0.9f});
    treeRenderer.setLightDirection({-0.3f, 0.3f, 0.9f});
    ...
    while (running)
    {
        ...
        while (focus && now - prev_time > frame_period)
        {
            ...
            payload* payload = socket.getpayloads();
            ...
            if (!paused)
            {
                if (!crashed)
                {
                    ...
                    aircraft.setTargetRollAngle(payload->target_roll);
                    aircraft.setLeftWingAngle(payload->lwing_angle);
                    aircraft.setRightWingAngle(payload->rwing_angle);
                    aircraft.setBodyHeightTarget(payload->body_height);
                    aircraft.setGameState(payload->game_state);
                    aircraft.setGameState(payload->game_state);
            ...
            if (camera.viewChanged())
            {
                ...
                birdLeftWingRenderer.setView(view);
                birdRightWingRenderer.setView(view);
                treeRenderer.setView(view);
            ...
            {
                // Don't crash, just restart the flight
                aircraft.recover();
            ...
            birdLeftWingRenderer.draw();
            birdRightWingRenderer.draw();
            treeRenderer.draw();
